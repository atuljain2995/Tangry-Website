...
export const ProductDetail = ({ product, categoryUrl, categoryLabel }: ProductDetailProps) => {
  ...
  const cartItem = cart.items.find(
    (item) =>
      ...
      item.variantId === product.variants[selectedVariantIndex]?.id,
  );
  ...
  const renderTextBlocks = (blocks: string[], keyPrefix: string) => {
    ...
    return blocks.map((block, index) => {
      ...
      if (labelMatch) {
        ...
        return (
          <div key={`${keyPrefix}-${index}`} className="space-y-2">
            <h3 className="text-2xl font-bold text-gray-900 tracking-tight">{label}</h3>
      ...
      return (
        <p key={`${keyPrefix}-${index}`} className="text-gray-700 leading-8 whitespace-pre-line">
  ...
  const usageBlocks = product.usageInstructions ? parseTextBlocks(product.usageInstructions) : [];
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-12">
        ...
        <ol className="flex items-center text-sm min-w-0">
          {breadcrumbItems.map((item, index) => {
            ...
            return (
              <li
                key={index}
                className={`flex items-center min-w-0 ${isLast ? 'flex-shrink' : 'shrink-0'}`}
              >
                {index > 0 && (
        ...
        <div>
          ...
          <div className="flex items-start justify-between gap-4 mb-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className={`p-2.5 border rounded-full transition ${
                  isWishlisted
                    ...
                    : 'text-gray-500 border-gray-300 hover:text-[#D32F2F] hover:border-[#D32F2F]'
                }`}
              ...
              <button
                onClick={handleShare}
                className="p-2.5 border border-gray-300 rounded-full text-gray-500 hover:text-gray-900 hover:border-gray-500 transition"
          ...
          {/* Rating (only shown when there are reviews) */}
          ...
          {/* Short Description */}
          <p className="text-gray-600 leading-7 mb-4">{product.description.split('\n')[0]}</p>

          {/* Compact delivery hint (key info near the price) */}
          <div className="flex items-center gap-1.5 text-sm text-gray-600 mb-6">
            <Truck size={16} className="text-gray-400" />
            <span>Delivered in 3–7 business days across India</span>
          </div>

          {/* Variant / Weight Selector */}
          {product.variants.length > 1 && (
            <div className="mb-6">
              <p className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-3">Weight</p>
          ...
          {/* Primary CTAs: Add to Cart (or quantity) + Buy Now — equal width */}
          <div className="flex items-stretch gap-3 mb-6">
            {quantityInCart > 0 ? (
              <div className="flex-1 flex items-center justify-center gap-3 bg-orange-50 rounded-lg px-3 py-2">
                <button
                  onClick={() => updateQuantity(product.id, selectedVariant.id, quantityInCart - 1)}
                  className="w-9 h-9 flex items-center justify-center rounded-md bg-white border border-orange-200 text-orange-600 hover:bg-orange-100 transition"
                  aria-label="Decrease quantity"
                >
                  <Minus size={16} strokeWidth={2.5} />
                </button>
                <span className="text-base font-bold text-orange-700 min-w-[1.5rem] text-center">
                  {quantityInCart}
                </span>
                <button
                  onClick={() => {
            ...
            <button
              ...
              disabled={!selectedVariant.isAvailable || selectedVariant.stock === 0}
              className="flex-1 bg-[#D32F2F] text-white px-5 py-3.5 rounded-lg font-bold text-sm tracking-wide hover:bg-[#B71C1C] transition disabled:bg-gray-300 disabled:cursor-not-allowed"
            >
              Buy Now
            </button>
          </div>
        ...
        <div className="prose max-w-none space-y-8 sm:space-y-10">
          <section className="space-y-4">
            ...
            {descriptionBlocks.length > 0 ? (
              ...
              <p className="text-gray-700 leading-8 whitespace-pre-line">{product.description}</p>
            )}
          </section>
          ...
          {product.usageInstructions && (
            <section className="space-y-4">
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Usage</h3>
      ...
      {/* Spacer so content isn't hidden behind the mobile sticky bar */}
      <div className="h-20 md:hidden" aria-hidden />

      {/* Mobile sticky buy bar — keeps the primary CTA in view while scrolling */}
      <div
        className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur border-t border-gray-200 px-4 py-2.5 flex items-center gap-3 shadow-[0_-2px_10px_rgba(0,0,0,0.06)]"
      ...
      >
        ...
        <div className="flex-1 flex gap-2">
          {quantityInCart > 0 ? (
            <button
              onClick={openCart}
              className="flex-1 bg-orange-600 text-white py-3 rounded-lg font-bold text-sm hover:bg-orange-700 transition"
            >
            ...
            <button
              onClick={handleAddToCart}
              disabled={!selectedVariant.isAvailable || selectedVariant.stock === 0}
              className="flex-1 bg-orange-600 text-white py-3 rounded-lg font-bold text-sm hover:bg-orange-700 transition disabled:bg-gray-300 disabled:cursor-not-allowed"
            >
              Add to Cart
            </button>
          )}
          <button
            onClick={handleBuyNow}
            disabled={!selectedVariant.isAvailable || selectedVariant.stock === 0}
            className="flex-1 bg-[#D32F2F] text-white py-3 rounded-lg font-bold text-sm hover:bg-[#B71C1C] transition disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Buy Now
          </button>
        </div>
