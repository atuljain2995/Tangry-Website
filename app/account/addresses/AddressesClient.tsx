...
export function AddressesClient({ addresses }: { addresses: AccountAddressRow[] }) {
  ...
  async function removeAddress(id: string) {
    ...
    setDeletingId(null);
    if (r.success) {
      setMsg({ ok: true, text: 'Address removed.' });
      router.refresh();
    } else setMsg({ ok: false, text: r.error });
  }
    ...
    <div className="space-y-6">
      ...
      {addrDraft && (
        <form
          ...
          className="space-y-4 rounded-xl border border-orange-200 bg-orange-50/40 p-5 dark:border-orange-800 dark:bg-orange-950/30"
        >
      ...
      <ul className="space-y-3">
        {addresses.map((row) => (
          <li
            key={row.id}
            className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900 sm:flex-row sm:items-start sm:justify-between"
          >
