...
8. **006_tangry_catalog_alignment.sql** – _(optional)_ If you already ran an **older** 002/003 with placeholder products (garam masala, biryani, etc.), run this once in Supabase to replace them with the current Tangry catalog. **Skip** on fresh installs that use the updated 002/003.
9. **007_remove_eazy_chef_paneer_tikka.sql** – _(optional)_ Removes rows whose name contains **Eazy Chef** / **Easy Chef** (e.g. “Eazy Chef - Paneer Tikka Masala”) and deletes the legacy `paneer-tikka-masala-mix` product if it still exists.
