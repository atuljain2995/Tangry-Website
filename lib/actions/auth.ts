...
export async function signIn(email: string, password: string): Promise<SignInResult> {
  ...
  if (!normalizedEmail || !password) return { success: false, error: 'Email and password required' };

  const { data: rows } = await db.from('users').select('id, email, password_hash').eq('email', normalizedEmail);
