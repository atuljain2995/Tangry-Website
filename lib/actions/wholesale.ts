...
export async function submitWholesaleQuote(payload: SubmitWholesalePayload): Promise<SubmitWholesaleResult> {
  const { companyName, contactPerson, email, phone, gstNumber, businessType, estimatedMonthlyVolume, message } = payload;
  ...
  if (!/^[6-9]\d{9}$/.test(phone.trim())) return { success: false, error: 'Enter a valid 10-digit Indian mobile number' };
  ...
  if (estimatedMonthlyVolume?.trim()) messageParts.unshift(`Estimated monthly volume: ${estimatedMonthlyVolume.trim()}.`);
